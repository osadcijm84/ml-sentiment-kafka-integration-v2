...
kf_producer = None
try:
    kf_producer = KafkaProducerService(
        bootstrap_servers=auth_config.kafka_bootstrap_servers.split(","),
        topic=auth_config.kafka_topic
    )
...
def health_check():
    ...
    kafka_producer_initialized = kf_producer is not None and kf_producer.producer is not None
    ...
    return jsonify({
        ...
        \'kafka_producer_initialized\': kafka_producer_initialized,
        \'version\': \'2.0\'
    })
...
def predict_sentiment():
    """Predict sentiment with database logging and Kafka publishing"""
    try:
        # Get request data
        ...
        if kf_producer and kf_producer.producer:
            kafka_message = {
                "text": text,
                "sentiment": sentiment,
                "confidence": confidence,
            ...
            kf_producer.send_message(kafka_message)
        else:
            logger.warning("Kafka Producer not available. Prediction not sent to Kafka.")
